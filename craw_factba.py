...
def download_by_transcript(urls):
    ret = dict()
    has_not_trump_ = dict()
    global output_filename
    file_num = 184
    try:
        for url in urls:
            ...
            size = None
            al_not = e("div.media.topic-media-row.mediahover.not-trump")
            title_e = e("h1.topic-page-header.transcript-header")
            title = str(title_e[0].text_content())
            if len(al_not) == 0:
                al = e("div.media.topic-media-row.mediahover:not(.not-trump)")
                contents = []
                for l in al:
                    ...
                    if not youtube_url:
                        youtube_uri = "https://www.youtube.com/embed/"
                        dataset_id = str(l.xpath('.//div[@class="youtube-large"]')[0].attrib["data-id"])
                        youtube_url = youtube_uri + dataset_id  # youtube地址
                        (_, size) = download_youtube(youtube_url)  # 本地存储的视频地址 # 视频总大小
                ret.update({
                    file_name: {
                        "youtube_url": youtube_url,
                        "title": title,
                ...
                for l in al_not:
                    ...
                    if not youtube_url:
                        ...
                        dataset_id = str(l.xpath('.//div[@class="youtube-large"]')[0].attrib["data-id"])
                        youtube_url = youtube_uri + dataset_id  # youtube地址
                        (_, size) = download_youtube(youtube_url)  # 本地存储的视频地址 # 视频总大
